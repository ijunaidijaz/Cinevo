...
public class SettingMultiScreenActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        cbox_screen.setChecked(sharedPref.getIsScreen());

        screenDialog = new ScreenDialog(this, screen -> {
            sharedPref.setScreen(screen);
        ...
        findViewById(R.id.btn_select).setOnClickListener(v -> screenDialog.showDialog());

        findViewById(R.id.ll_btn_save).setOnClickListener(v -> {
            sharedPref.setIsScreen(cbox_screen.isChecked());
    ...
    private void setScreen() {
        int screen = sharedPref.getScreen();
        if (screen == 1){
